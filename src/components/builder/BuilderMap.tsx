...
export default function BuilderMap({ lat, lng, name, address }: Props) {
  ...
  useEffect(() => {
    ...
    el.style.cssText = `
      ...
      border-radius: 50%;
      box-shadow: 0 0 0 4px rgba(42,171,171,0.25), 0 4px 12px rgba(0,0,0,0.5);
      cursor: pointer;
        ...
        new mapboxgl.Popup({ offset: 20, closeButton: false })
          .setHTML(`
            <div style="font-family: Inter, system-ui, sans-serif; padding: 4px 2px;">
              <p style="font-weight: 700; font-size: 13px; color: #F0EDE4; margin: 0 0 2px;">${name}</p>
              ${address ? `<p style="font-size: 11px; color: rgba(240,237,228,0.5); margin: 0;">${address}</p>` : ''}
            </div>
          `)
      )
      .addTo(map)
