...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const meta = getCategoryMetaForLocale(locale)['guide']
  return {
    title: meta.title,
...
export default async function GuidePage() {
  ...
  const locale = await getLocale()
  const articles = getArticlesByCategoryForLocale('guide', locale)
  const meta = getCategoryMetaForLocale(locale)['guide']

  const CATEGORY_TABS = [
    { label: t('tabAll'),          href: '/magazine' as const },
    { label: t('tabBuildStories'), href: '/magazine/build-story' as const },
    { label: t('tabInterviews'),   href: '/magazine/interview' as const },
    { label: t('tabGuides'),       href: '/magazine/guide' as const },
  ]
    ...
    <div className="min-h-screen bg-[#F7F7F7] text-[#222222]">
      ...
      <section className="pt-28 pb-10 bg-white border-b border-[#222222]/6">
        <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 text-center">
          <p className="text-[10px] font-bold text-[#717171] uppercase tracking-[0.2em] mb-3">{t('kicker')}</p>
          <h1
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xs font-bold text-[#222222]/40 uppercase tracking-widest">{t('tabGuides')}</h2>
            <span className="text-xs text-[#222222]/25">{t('postsCount', { count: articles.length })}</span>
          </div>

          {articles.length === 0 ? (
            <p className="text-[#222222]/40 text-sm py-20 text-center">{t('noArticlesInCategory')}</p>
          ) : (
