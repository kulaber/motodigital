...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: '/ueber-motodigital' },
  }
}
...
export default async function WieEsFunktioniertPage() {
  ...
  const sections = [
    {
      ...
      uspCount: 5,
      cta: { href: '/auth/register?role=custom-werkstatt' },
      accent: '#06a5a5',
    },
    {
      number: '02',
    ...
    },
    {
      ...
      key: 'riders' as const,
      uspCount: 7,
      cta: { href: '/auth/register?role=rider' },
      accent: '#06a5a5',
    },
  ]

  const roadmapItems = [
    { statusKey: 'live' as const, live: true },
    { statusKey: 'live' as const, live: true },
    { statusKey: 'live' as const, live: true },
    { statusKey: 'soon' as const, live: false },
    { statusKey: 'soon' as const, live: false },
    { statusKey: 'planned' as const, live: false },
    { statusKey: 'planned' as const, live: false },
    { statusKey: 'planned' as const, live: false },
    { statusKey: 'planned' as const, live: false },
  ]
  return (
    <>
      ...
      {/* Sections */}
      {sections.map((s, i) => {
        const tag = t(`sections.${s.key}.tag`)
        const heading = t(`sections.${s.key}.heading`)
        const description = t(`sections.${s.key}.description`)
        const ctaLabel = t(`sections.${s.key}.cta`)
        const usps = Array.from({ length: s.uspCount }, (_, idx) =>
          t(`sections.${s.key}.usps.${idx}`)
        )
      ...
      <section className="bg-[#222222]" id="roadmap">
        <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 py-24">
          ...
          {/* Heading */}
          <div className="mb-16">
            <p className="text-xs font-bold uppercase tracking-widest text-[#06a5a5] mb-4">{t('roadmap')}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5">
            {roadmapItems.map((item, idx) => {
              const title = t(`roadmapItems.${idx}.title`)
              const description = t(`roadmapItems.${idx}.description`)
              const statusLabel = t(`roadmapStatus.${item.statusKey}`)

              return (
                <div key={title} className="bg-[#222222] p-8 flex flex-col gap-4">
      ...
      <section className="bg-[#111111] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 py-24 text-center">
          <p className="text-xs font-bold uppercase tracking-widest text-[#06a5a5] mb-4">{t('ctaKicker')}</p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/auth/register?role=custom-werkstatt"
              ...
              {t('ctaStartFree')} <ArrowRight size={14} />
            </Link>
            <Link href="/auth/register?role=rider"
              ...
              {t('ctaRegisterRider')} <ArrowRight size={14} />
            </Link>
          </div>
        </div>
      </section>
