...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('WerkstattPage')
  const seoTitle = t('metaTitle')
  const seoDescription = t('metaDescription')
  return {
    title: seoTitle,
...
export default async function BuilderPage() {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: t('heading') },
    ],
  }
  ...
  return (
    <div className="min-h-screen bg-white text-[#222222]">
      ...
      <div className="pt-6 pb-4 px-4 sm:px-5 lg:hidden">
        <h1 className="text-xl font-bold text-[#222222] text-center">{t('heading')}</h1>
      </div>
          ...
          <div className="sticky top-12 lg:top-16 z-30 bg-white/95 backdrop-blur-md border-b border-[#222222]/5">
            <div className="flex px-4 sm:px-5 lg:px-6 py-3 items-center gap-2 overflow-x-auto">
              ...
              <div className="hidden lg:flex h-8 items-center gap-1.5 px-3.5 rounded-full border border-[#d4d4d4] flex-shrink-0">
                ...
                <span className="text-[13px] font-medium text-[#333]">{t('openNow')}</span>
              </div>
            </div>
          </div>
          {/* Desktop split */}
