...
function LoginFormInner() {
  ...
  if (magicSent) return (
    <div className="text-center py-4">
      <p className="text-sm text-white mb-1">{t('linkSent')}</p>
      <p className="text-xs text-white/40">{t('linkSentText')}</p>
    </div>
  )
    ...
    <form onSubmit={handlePassword} className="flex flex-col gap-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label className="block text-xs font-semibold text-white/50">{t('password')}</label>
          {resetSent ? (
            <span className="text-[11px] text-green-500">{t('resetSent')}</span>
          ) : (
            <button
              type="button"
              onClick={handleReset}
            ...
            >
              {resetLoading ? t('resetSending') : t('forgotPassword')}
            </button>
          )}
        </div>
        <input
