...
export async function saveEvent(formData: FormData) {
  ...
  // i18n payloads — admin submits per-locale fields; fall back to DE fields
  // to preserve old single-locale behaviour when EN is empty.
  const nameEn        = (formData.get('name_en') as string)?.trim() ?? ''
  const descriptionEn = (formData.get('description_en') as string)?.trim() ?? ''
  const locationEn    = (formData.get('location_en') as string)?.trim() ?? ''

  const buildI18n = (de: string, en: string): Record<string, string> | null => {
  ...
  const payload = {
    ...
    videos,
    // Localized mirrors (migration 073). Legacy text fields stay in sync
    // with the DE value above for readers that haven't migrated yet.
    name_i18n:        buildI18n(name, nameEn),
    description_i18n: buildI18n(description, descriptionEn),
    location_i18n:    buildI18n(location, locationEn),
  }
