        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => fileInput.current?.click()} disabled={uploading}
              className="flex items-center gap-1.5 text-xs bg-[#2AABAB]/10 border border-[#2AABAB]/25 text-[#2AABAB] px-3 py-2 rounded-full hover:bg-[#2AABAB]/20 transition-all disabled:opacity-40">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {media.map(item => (
              <div key={item.id} className="group relative rounded-xl overflow-hidden bg-[#141414] border border-[#F0EDE4]/6">
                ...
                {item.type === 'video' && (
                  <span className="absolute top-2 left-2 bg-[#2AABAB]/90 text-[#141414] text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase">
                    Video
                  </span>
                )}
              </div>
