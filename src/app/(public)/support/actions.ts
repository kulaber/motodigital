...
export async function submitContactForm(formData: FormData) {
  ...
  if (emailError) {
    ...
    return { error: { _form: ['E-Mail konnte nicht gesendet werden. Bitte versuche es später erneut.'] } }
  }

  // Save to DB as backup (non-blocking — don't fail if DB insert fails)
  try {
    const supabase = await createClient()
    await supabase.from('contact_submissions' as never).insert(parsed.data as never)
  } catch (e) {
    console.error('[Contact] DB insert error:', e)
  }
