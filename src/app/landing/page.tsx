      ...
      <section className="relative min-h-screen flex flex-col justify-end pb-16 lg:pb-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 lg:px-8 w-full">
          <div className="max-w-2xl">
            ...
            <h1
              className="animate-slide-up font-bold text-[#F0EDE4] leading-[1.1] mb-5"
              style={{ fontSize: 'clamp(2rem, 4.5vw, 3.6rem)', animationDelay: '60ms' }}
            >
              Entdecke die Welt<br />der Custom Bikes
            </h1>
            <p
              className="animate-slide-up text-[#F0EDE4]/55 font-light mb-8 leading-relaxed"
        ...
        <div className="max-w-6xl mx-auto px-5 lg:px-8">
          <AnimateIn className="flex items-end justify-between mb-10 gap-4">
            <div>
              ...
              <h2 className="font-bold text-[#F0EDE4] leading-tight" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.6rem)' }}>
                Handgefertigte Unikate.<br />
                <span className="text-[#F0EDE4]/25">Aus der Community.</span>
              </h2>
            </div>
            <Link href="/builds" className="flex-shrink-0 border border-[#F0EDE4]/15 text-[#F0EDE4]/60 hover:text-[#F0EDE4] hover:border-[#F0EDE4]/30 text-sm font-medium px-5 py-2.5 rounded-full transition-colors duration-200">
              Alle ansehen →
            </Link>
          </AnimateIn>
              ...
              <AnimateIn key={build.title} delay={i * 60}>
                <Link href="/map"
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#141414]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {USPS.map((usp, i) => (
              <AnimateIn key={usp.title} delay={i * 70}>
                <div className="bg-[#141414] border border-[#F0EDE4]/6 rounded-2xl p-6 hover:border-[#2AABAB]/20 transition-colors duration-200 h-full">
      ...
      <section className="py-20 lg:py-28 bg-[#141414]" id="builder">
        <div className="max-w-6xl mx-auto px-5 lg:px-8">
          <AnimateIn className="flex items-end justify-between mb-10 gap-4">
            ...
            <Link href="/builder" className="flex-shrink-0 border border-[#F0EDE4]/15 text-[#F0EDE4]/60 hover:text-[#F0EDE4] hover:border-[#F0EDE4]/30 text-sm font-medium px-5 py-2.5 rounded-full transition-colors duration-200">
              Alle Builder →
            </Link>
          </AnimateIn>
            ...
            {BUILDERS.slice(0, 3).map((b, i) => (
              <AnimateIn key={b.slug} delay={i * 70}>
                <Link href={`/builder/${b.slug}`}
                  className="group bg-[#1C1C1C] border border-[#F0EDE4]/6 rounded-2xl p-5 hover:border-[#2AABAB]/25 hover:-translate-y-0.5 transition-all duration-200 block h-full">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="text-sm font-semibold text-[#F0EDE4] group-hover:text-[#2AABAB] transition-colors duration-200">{b.name}</p>
      ...
      <section className="py-20 lg:py-28 bg-[#1C1C1C] relative overflow-hidden" id="waitlist">
        ...
        <AnimateIn className="max-w-md mx-auto px-5 relative z-10">
          <div className="text-center mb-8">
            ...
            <h2 className="font-bold text-[#F0EDE4] mb-3" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.4rem)' }}>
  ...
  return (
    <form action="/api/waitlist" method="POST" className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="name" className="block text-[10px] font-semibold text-[#F0EDE4]/35 uppercase tracking-widest mb-1.5">Name</label>
          <input id="name" name="name" type="text" placeholder="Dein Name" required
            className="w-full bg-[#1C1C1C] border border-[#F0EDE4]/10 rounded-xl px-3 py-2.5 text-sm text-[#F0EDE4] placeholder:text-[#F0EDE4]/20 outline-none focus:border-[#2AABAB] transition-colors duration-200 min-h-[44px]" />
        </div>
        <div>
          <label htmlFor="email" className="block text-[10px] font-semibold text-[#F0EDE4]/35 uppercase tracking-widest mb-1.5">E-Mail</label>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[['builder','Builder / Workshop'],['rider','Rider']].map(([val, label]) => (
            <label key={val} className="cursor-pointer">
              ...
              <div className="border border-[#F0EDE4]/10 rounded-xl px-4 py-3 text-center text-sm font-medium text-[#F0EDE4]/50 hover:border-[#2AABAB]/40 transition-colors duration-200 has-[:checked]:border-[#2AABAB] has-[:checked]:text-[#2AABAB] has-[:checked]:bg-[#2AABAB]/08 min-h-[44px] flex items-center justify-center">
                {label}
              </div>
            </label>
          ))}
